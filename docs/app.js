...
function pollView() {
  ...
  return frame(`
    ...
    <div class="page-title">
      ...
      ${admin ? `<div class="button-row"><button class="btn" data-export>⇩ Export campaign</button><button class="btn" data-modal="import">↑ Import voters</button><button class="btn primary" data-view="team">＋ Invite team member</button></div>` : ""}
    </div>
...
function exportsView() {
  ...
  return frame(`<div class="page-title"><div><span class="eyebrow">Admin reporting</span><h2>Exports</h2><p>Download the current campaign's election-day records.</p></div></div>
    <section class="card section-card"><div class="export-box"><div><h3>Campaign voter export</h3><p>Exports this browser's demo records; includes town, precinct, voted status, and recorded consent.</p></div><button class="btn primary" data-export>⇩ Download CSV</button></div>
    ...
    <section class="card section-card queue"><h3>Message queue</h3><p style="color:var(--muted);font-size:12px">Demo simulation only. No text messages are sent.</p>${messages.length ? messages.map(m => `<div class="queue-row"><div><strong>${esc(m.recipient)}</strong><small>${esc(m.kind)} · ${new Date(m.scheduledFor).toLocaleString()}</small></div><span class="queued">Demo only</span></div>`).join("") : `<div class="empty">No messages have been queued.</div>`}</section>`);
}
...
function modalMarkup() {
  ...
  return `<div class="modal-backdrop" data-close-modal><div class="modal" onclick="event.stopPropagation()"><div class="modal-head"><div><h3>Import voter records</h3><p style="color:var(--muted);font-size:12px">Use the included CSV template or upload your own compatible file.</p></div><button class="close" data-close-modal>×</button></div>
    <div class="notice"><strong>Do not import real voter data.</strong> This public demo stores imported records only in this browser, without production security or team synchronization.</div><div class="button-row" style="margin-top:14px"><button class="btn primary" id="choose-csv">Choose CSV</button><a class="btn" href="sample-voter-import.csv" download>Download template</a></div>
...
app.addEventListener("click", async event => {
  if (event.target.closest("[data-export]")) { window.demoExport(); return; }
  ...
  if (checkin) { try { await request("api/checkin", { method: "POST", body: JSON.stringify({ voterId: checkin.dataset.checkin }) }); flash("Demo check-in saved in this browser."); await refresh(); } catch (e) { flash(e.message); } return; }
  const message = event.target.closest("[data-message]");
  if (message) { try { const delay = document.querySelector("#delay")?.value || 0; const result = await request("api/messages", { method: "POST", body: JSON.stringify({ voterId: message.dataset.voterId, target: message.dataset.message, delay }) }); state = result.state; flash(`${result.queued} demo message${result.queued === 1 ? "" : "s"} simulated; nothing sent.`); render(); } catch (e) { flash(e.message); } return; }
...
app.addEventListener("submit", async event => {
  ...
  if (event.target.id === "invite-form") {
    ...
    try { const invite = await request("api/invites", { method: "POST", body: JSON.stringify(data) }); const result = document.querySelector("#invite-result"); result.className = "invite-result"; result.innerHTML = `<strong>Demo quicklink only — it does not grant real access</strong><code>${esc(invite.url)}</code><button class="btn" id="copy-link" type="button" style="margin-top:10px">Copy link</button>`; document.querySelector("#copy-link").onclick = async () => { await navigator.clipboard.writeText(invite.url); flash("Demo link copied."); }; } catch (e) { flash(e.message); }
  }
  if (event.target.id === "settings-form") {
    ...
    try { await request("api/settings", { method: "POST", body: JSON.stringify(data) }); flash("Demo settings saved in this browser."); await refresh(); } catch (e) { flash(e.message); }
  }
});
